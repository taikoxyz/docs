...
First, you will need to have a prover from the marketplace. This prover must be able to generate SGX proofs. See the current tier configuration [here](/network-reference/network-configuration).

Then you can set `PROVER_ENDPOINTS` in simple-taiko-node `.env` file to the prover's endpoint.
...
2. Submit your public key to the form: https://forms.gle/qquRq3cnGbyneV9S6. After submitting we will manually register the public key and send you the necessary TTKOk.
