...
- Meet the <a href="https://github.com/ethereum/go-ethereum#hardware-requirements" target="_blank" rel="noopener noreferrer">Geth minimum hardware requirements</a> except for the storage requirement because Taiko nodes will require less storage (at the time of writing).
...
3. Set the L1 archive node endpoint
    ...
    It's recommended to <a href="/guides/run-a-holesky-node" target="_blank" rel="noopener noreferrer">run a local Holesky node</a> but you browse around for other <a href="https://chainlist.org/chain/17000" target="_blank" rel="noopener noreferrer">Holesky RPC providers</a>. Keep in mind they will **eventually rate limit your node** and it will stop syncing, so a local L1 node is recommended for a proper setup.
    :::
    ...
    - Using `host.docker.internal` (see: <a href="https://stackoverflow.com/questions/24319662" target="_blank" rel="noopener noreferrer">stack overflow</a>).
...
6. Verify node is running
    ...
    A node dashboard will be running on `localhost` on the `GRAFANA_PORT` you set in your `.env` file, which defaults to `3001`: (<a href="http://localhost:3001/d/L2ExecutionEngine/l2-execution-engine-overview" target="_blank" rel="noopener noreferrer">http://localhost:3001/d/L2ExecutionEngine/l2-execution-engine-overview</a>).
...
See the video tutorial <a href="https://www.youtube.com/watch?v=l_tVvDcuK5o" target="_blank" rel="noopener noreferrer">Run a Taiko L2 node (YouTube)</a>. It's for a previous testnet but the overall steps are the same!
